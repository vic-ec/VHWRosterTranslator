...
// A first initial standing alone as its own text item. Whether an export sets
// "M. Willemse" as one item or as "M." followed by "Willemse" is up to the
// PDF; the single-item spelling is read by INITIAL_RE below, and this is the
// other one. Deliberately one letter and a full stop only — "St." is two, and
// a prefix like "Van" carries no stop, so neither can be mistaken for this.
const INITIAL_TOK_RE = /^[A-Z]\.$/;
const nameClean = s => (s||'').replace(/\(T\)/gi,'').replace(/\(Psy\)/gi,'').replace(/\[|\]/g,'').trim();
...
function extractNamesWithAnchors(rowWords, anchorXs, maxDist) {
  ...
  while (i < sorted.length) {
    ...
    // The same name split across two text items: "M." and then "Willemse".
    // Without this the lone initial fails isNameTok, is skipped, and the two
    // doctors who share that surname merge into one carrying both their
    // shifts — a silent doubling of one person's hours and the loss of the
    // other. The 120px gap is the one the compound-name join already uses.
    if (INITIAL_TOK_RE.test(raw) && i+1 < sorted.length) {
      const nw = sorted[i+1], nraw = nameClean(nw.text);
      if (isNameTok(nraw) && nw.x - w.x < 120) {
        const col = nearestCol(w.x, anchorXs, maxDist);
        if (col >= 0) out.push({ name: raw + ' ' + nraw, col });
        i += 2; continue;
      }
    }
...
async function parseRosterPDF(arrayBuffer) {
  ...
  for (let p = 1; p <= pdf.numPages; p++) {
    ...
    for (const y of sortedYs) {
      ...
      if (leaveX != null) {
        // Sorted by x so a split initial can see the surname that follows it,
        // the same join extractNamesWithAnchors does. An export that splits
        // them in the shift columns splits them here too.
        const lw = row.filter(w => Math.abs(w.x - leaveX) <= LEAVE_MATCH_DIST)
                      .sort((a,b) => a.x - b.x);
        for (let k = 0; k < lw.length; k++) {
          const raw = nameClean(lw[k].text);
          ...
          const nraw = k+1 < lw.length ? nameClean(lw[k+1].text) : '';
          if (compoundMatch) name = compoundMatch[1] + ' ' + compoundMatch[2];
          else if (initialMatch) name = initialMatch[1] + ' ' + initialMatch[2];
          else if (INITIAL_TOK_RE.test(raw) && isNameTok(nraw) && lw[k+1].x - lw[k].x < 120) {
            name = raw + ' ' + nraw; k++;
          }
