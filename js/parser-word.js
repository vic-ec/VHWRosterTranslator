...
function getTableShifts(tableData, doctorName, targetMonth, profile, targetYear) {
  ...
  if (typeof getSAPublicHolidays === 'function') {
    ...
    for (let d = 1; d <= 31; d++) {
      const dt = new Date(phYear, targetMonth, d);
      if (dt.getMonth() !== targetMonth) continue;
      const key = phYear + '-' + String(dt.getMonth() + 1).padStart(2, '0') + '-' +
  ...
  for (const day of tableData.days) {
    ...
    // Which role is this doctor filling today? First match wins, in the
    // profile's declared column order.
    let role = null, note = '';
    for (const key of Object.keys(profile.table.role_columns || {})) {
      const hit = (day.roles[key] || []).find(p => p.name.toLowerCase() === nl);
      if (hit) { role = key; note = hit.note; break; }
    }
    if (!role) continue;

    const roleRule = rules[role];
    if (!roleRule) continue;

    const isPH = phDays.has(day.date);
    const isSpecial = day.isWeekend || isPH;
    const rule = isSpecial ? roleRule.weekend_ph : roleRule.weekday;
    if (!rule) continue;

    const label = isPH ? (roleRule.label_ph || `${role} - Public Holiday`)
      : day.isWeekend ? (roleRule.label_weekend || `${role} - Weekend`)
        : (roleRule.label_weekday || `${role} - Weekday`);
